...
__global__ void mb_kernel(float* data, int cx, int cy, float fcx, float fcy, float scale, int n_iters, int n_rows) {
    int r = blockIdx.y * blockDim.y + threadIdx.y;
    int c = blockIdx.x * blockDim.x + threadIdx.x;
    data[r*n_rows + c] = do_mb(screen_space_to_complex(c, r, cx, cy, fcx, fcy, scale), n_iters);
}
...
void calculate_mb(std::vector<std::vector<float>>& in_v, const Point center, const float scale, const int n_iters) {
    ...
    mb_kernel<<<numBlocks, threadsPerBlock>>>(dev_data, x_c, y_c, center.x, center.y, scale, n_iters, n_rows);
    cu_assert(cudaDeviceSynchronize());
    for (int r = 0; r < n_rows; r++) {
	cu_assert(cudaMemcpy(in_v[r].data(), dev_data + r * n_cols, n_cols, cudaMemcpyDeviceToHost));
    }
